...
void MDAEPiano::update() noexcept
{
    ...
    _envDecay = _params.envDecayParam->get() / 100.0f;
    ...
    _modulation = _params.modulationParam->get();
    ...
    float param5 = _params.lfoRateParam->get();
...
void MDAEPiano::noteOn(int note, int velocity) noexcept
{
    if (velocity > 0) {
        // === Find voice ===
